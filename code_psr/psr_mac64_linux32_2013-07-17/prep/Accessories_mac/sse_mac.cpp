...
void sse_conj(void *A, int cnt)
{
	...
	if((long)A%16)  // changed by Junlan on 2013_07_12, previous centence here is if((int)A%16) 
	{
		...
		(
			...
			"jecxz Z%=						\n\t"
			 ...
			 "LOOP%=:							\n\t"  // change by junlan on 2013_07_12 to meet mac os format for loop, previous sentence here is " L%=:     \n\t"
			...
			//"loop L%=						\n\t"// Loop if not done, commented by junlan on 2013_07_12 to meet mac os format for loop
			"Z%=:							\n\t"

			"mov ecx, [ebp-12]				\n\t"// Counter
			"jecxz ZZ%=						\n\t"
			"LOOP2%=:						\n\t" //change by junlan on 2013_07_12 to meet mac os format for loop, previous sentence here is " LL%=:     \n\t"
         ...
         //"loop LL%=						\n\t" //commented by junlan on 2013_07_12 to meet mac os format for loop
...
int64 sse_cacc(void *A, void *B, int cnt)
{
	...
	if(((long)A%16) || ((long)B%16)) // changed by Junlan on 2013_07_12, previous centence here is if(((int)A%16) || ((int)B%16)) 
	{
...
void sse_mulc(void *A, void *B, void *C, int cnt)
{
	...
	if(((long)A%16) || ((long)B%16) || ((long)C%16)) // changed by Junlan on 2013_07_12, previous centence here is if(((int)A%16) || ((int)B%16)) 16)) || ((int)C%16)
	{
	...
	"Z%=:					\n\t"
			...
			"LOOP2%=:						\n\t"	//Really finish off loop with non SIMD // change by junlan on 2013_07_12 to meet mac os format for loop, previous sentence here is " LL%=:     \n\t"instructions
		...
		else
		{
			__asm
				...
				"Z%=:					\n\t"
						...
						"LOOP2%=:						\n\t"	//Really finish off loop with non SIMD instructions // change by junlan on 2013_07_12 to meet mac os format for loop, previous sentence here is " LL%=:     \n\t"
