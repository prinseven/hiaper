...
OLChanBck::OLChanBck(int _Chan_number) :
 ...
 *
 ---------------------------------------------------------------------------------------------*/
void OLChanBck::doStartChannel(const OL_Frame_Start_S &FrameInfo)
{
	iSV = FrameInfo.SV;
...
//	Data = fopen("testdata.bin","wb");
	...
	_1ms_epoch = 0;
	_20ms_epoch = 299;
	z_count = FrameInfo.z_count-4;
    printf("FrameInfo.z_count=%d\n",FrameInfo.z_count);
    printf("z_count=%d\n",int(z_count));
	// Open the appropriate input/output files.
	char fname[50];

	sprintf(fname,"OLPredictSV%02dr.in",iSV+1);
	...
	sprintf(fname,"ClimateSV%02dr.in",iSV+1);
	...
	sprintf(fname,"OLPredictSV%02dr.out",iSV+1);
 ...
 *
 ---------------------------------------------------------------------------------------------*/
void OLChanBck::doPredictIn(unsigned short _GPS_Wk, unsigned long _GPS_MSOW)
{
	...
	if (ClimateIn == NULL)
	{
        while (!feof(PredictIn) && !((unsigned long)(_GPS_MSOW/1000) == FreqPrediction.GPS_SOW))
        {
	...
	else
	{
	    while (!feof(PredictIn) && !((unsigned long)(_GPS_MSOW/1000) == FreqPrediction.GPS_SOW))
        {
            ...
            // Read a block of the input file.
            fread(&FreqClimate,sizeof(OL_Predict_S),1,ClimateIn);

        }
        //printf("the climatate freq = %f", FreqClimate.FreqClimateHz[0]);
	}
	...
	*/
	...
	if (_GPS_Wk == FreqPrediction.GPS_Week && _GPS_MSOW/1000 == FreqPrediction.GPS_SOW) {
		...
		else
		{
		    ...
		    /*if(FreqClimate.FreqClimateHz[_GPS_MSOW % 1000]!=0)
		    {
		        printf("the climatate freq = %f", FreqClimate.FreqClimateHz[_GPS_MSOW % 1000]);
		    }*/
...
// OUTPUT VALUES TO A FILE
		...
		fprintf(PredictOut,"%d,%lu,%lu,%f,%d,%d,%f,%f,%d,%f",
			GPS_Week,
			GPS_MSOW,
			MsecHeader.GPS_MSOW + 1, // HACK:  because the pointer was moved to the previous MSEC and we want the current MSEC
			FreqPredictHz,
			Phasor.r,
			Phasor.i,
			PhiResidual,
 ...
 *
 ---------------------------------------------------------------------------------------------*/
void OLChanBck::doCodePhase(void)
{
	...
	FracCodePhaseChips -= ChipsPerSample-FracChipsLeft;
	CycleEdgeIndex--;

	// if the code cycle moved to previous sample
	if(FracCodePhaseChips < 0){
 ...
 *
 ---------------------------------------------------------------------------------------------*/
void OLChanBck::doCodeRate(double FreqBase)
{
 ...
 *
 ---------------------------------------------------------------------------------------------*/
void OLChanBck::doCarrierPhase(int NumSamples)
{
 ...
 *
 ---------------------------------------------------------------------------------------------*/
big_cpx OLChanBck::doCarrierWipe() const
{
	big_cpx result;
//	CPX_ACCUM corr_temp;
	int64 corr_temp;
...
bool OLChanBck::BufferFill()
{
	...
	if (NumSamplesStored == 0) {
		...
		DATA_CPX *CycleCurr = &CycleBuff[D_SAMPLES_PER_MS-CycleEdgeIndex];
		// How many samples could we get out of this data set?
		NumSamplesAvail = CycleEdgeIndex-(D_SAMPLES_PER_MS-NumSamplesInCycle);

		// If there are no samples available at all, then return.
		if (NumSamplesAvail <= 0) {
			DATA_CPX *CycleCurr = &CycleBuff[0];
			...
			CycleEdgeIndex = NumSamplesAvail+2;
			NumSamplesStored=NumSamplesRemaining;
			// Buffer still needs information and there's nothing in this data set we can use!
			// Extract the GPS time from the header.
			ms_corr=true;

			if(c_e_corr){
				GPS_MSOW_PRV =GPS_MSOW+1;
			}else{
				GPS_MSOW_PRV =GPS_MSOW+1;
			}
		...
		if (NumSamplesAvail >= NumSamplesRemaining) {
			...
			NumSamplesStored += NumSamplesRemaining;
			CycleEdgeIndex = NumSamplesInCycle-NumSamplesRemaining+2;
			if (CycleEdgeIndex > 10001) {
						// Reset the cycle edge index to start in the next data set.
						CycleEdgeIndex =1;// CycleEdgeIndex % D_SAMPLES_PER_MS;
 ...
 *
/---------------------------------------------------------------------------------------------*/
void OLChanBck::doGenSine(double _FrequencyHz, double _PhaseOffsetCycles, cpx_sine *_SineWave, int _SampleLength) const
{
